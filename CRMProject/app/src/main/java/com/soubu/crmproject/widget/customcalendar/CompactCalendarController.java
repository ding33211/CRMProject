package com.soubu.crmproject.widget.customcalendar;
...
class CompactCalendarController {
    ...
    private int calenderSelectedTextColor;
    ...
    private void loadAttributes(AttributeSet attrs, Context context) {
        if (attrs != null && context != null) {
            ...
            try {
                ...
                calenderSelectedTextColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalendarSelectedTextColor, calenderSelectedTextColor);
    ...
    void setUseWeekDayAbbreviation(boolean useThreeLetterAbbreviation) {
        ...
        if (useThreeLetterAbbreviation) {
            ...
            if (!shouldShowMondayAsFirstDay) {
                this.dayColumnNames = new String[]{dayNames[1].substring(1, 2), dayNames[2].substring(1, 2),
                        dayNames[3].substring(1, 2), dayNames[4].substring(1, 2), dayNames[5].substring(1, 2), dayNames[6].substring(1, 2), dayNames[7].substring(1, 2)};
            } else {
                this.dayColumnNames = new String[]{dayNames[2].substring(1, 2), dayNames[3].substring(1, 2),
                        dayNames[4].substring(1, 2), dayNames[5].substring(1, 2), dayNames[6].substring(1, 2), dayNames[7].substring(1, 2), dayNames[1].substring(1, 2)};
            }
        }
    }
        ...
        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                if (isSameYearAsToday && isSameMonthAsToday && todayDayOfMonth == day && !isAnimatingWithExpose) {
                    ...
                    dayPaint.setColor(calenderSelectedTextColor);
                    canvas.drawText("今", xPosition, yPosition, dayPaint);
                    dayPaint.setColor(calenderTextColor);
                }
...
//                }
                if (day <= monthToDrawCalender.getActualMaximum(Calendar.DAY_OF_MONTH) && day > 0
                        && !(isSameYearAsToday && isSameMonthAsToday && todayDayOfMonth == day && !isAnimatingWithExpose)) {
                    canvas.drawText(String.valueOf(day), xPosition, yPosition, dayPaint);
                }
            }
        }
