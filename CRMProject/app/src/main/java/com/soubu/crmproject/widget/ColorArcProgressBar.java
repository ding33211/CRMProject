...
public class ColorArcProgressBar extends View {
    ...
    private void initCofig(Context context, AttributeSet attrs) {
        ...
        bgArcWidth = a.getDimension(R.styleable.ColorArcProgressBar_back_width, dipToPx(5));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isNeedContent) {
            canvas.drawText(String.format("%.0f", curValues) + "%", centerX, centerY + 2 * textSize / 3 , vTextPaint);
        }
        if (isNeedUnit) {
            canvas.drawText(hintString, centerX, centerY + 2 * textSize / 3, hintPaint);
        }
        if (isNeedTitle) {
            canvas.drawText(titleString, centerX, centerY - 1 * textSize / 3, curSpeedPaint);
        }
    ...
    public void restartAnimation(int start, int end){
        setAnimation(start * k, end * k, aniSpeed);
    }
